  ...
  fetch(`/movies/${imdbID}`, { method: 'PUT' })
    .then(response => {
      if (response.status === 201) {
        const entry = document.querySelector(`[data-imdbid="${imdbID}"]`);
  ...
  fetch(`/search?query=${encodeURIComponent(query)}`)
    ...
    .then(results => {
      ...
      for (const movie of results) {
        const entry = new ElementBuilder("div").with("data-imdbid", movie.imdbID);
        const label = `${movie.Title}${movie.Year ? ' (' + movie.Year + ')' : ''}`;
        new ElementBuilder("span").text(label).appendTo(entry.element);
        new ButtonBuilder("Add").onclick(() => addMovie(movie.imdbID)).appendTo(entry.element);
